...
def test_worker_loop_success():
    port = MockToolPort()
    ai = MockAIPort('{"tool_name": "good_tool", "arguments": {}}')
    loop = WorkerLoop(ai, port)
    session = WorkerSession(objective="Do work")
    wi = WorkItem(id="wi1", objective="Do work", allowed_tools=["good_tool"], parameters={})
...
def test_worker_loop_unauthorized_tool():
    port = MockToolPort()
    ai = MockAIPort('{"tool_name": "bad_tool", "arguments": {}}')
    loop = WorkerLoop(ai, port)
    session = WorkerSession(objective="Do work")
    wi = WorkItem(id="wi2", objective="Do work", allowed_tools=["good_tool"], parameters={})
    
    result = loop.execute_work_item(session, wi)
    
    assert result.success is False
    assert "not in allowed tools" in result.findings
